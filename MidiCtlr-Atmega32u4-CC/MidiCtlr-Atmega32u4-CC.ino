...
----------------
  NOTE DE MAX BRUCKERT : 
  1/ code modifié pour que les entrées analogiques envoies du CC, pas des notes  Se reporter à au patche [MidiCtlr-Atmega32u4-NOTES].
    les entrées analogiques A0 à A5 == CC1 à 6 / entrees digitales 2 à 7 == CC7 à 12.
  2/ penser à changer le nombres d'entrees analogiques. Sinon on a des parasites pourries qui font moduler toutes les entrees.
    Editer la parties potentionmetres en fontion de nombres de capteurs connectés.
*/
...
const int potPin[NPots] = {A0, A1}; //*** define Analog Pins connected from Pots to Arduino; Leave nothing in the array if 0 pots {}
...
// MIDI Assignments 
byte midiCh = 0; //* MIDI channel to be used -1 so it corresponds to Ch1
byte note = 7; //* Digit inputs remapped to CC7… to follow analog ones
      ...
      if (buttonPState[i] != buttonCState[i]) {
        ...
        if (buttonCState[i] == LOW) {
         ...
         // use if using with ATmega32U4 (micro, pro micro, leonardo...)
          controlChange(midiCh, note + i, 127);  // channel, note, velocity
          MidiUSB.flush();
          ...
          // use if using with ATmega32U4 (micro, pro micro, leonardo...)
          controlChange(midiCh, note + i, 0);  // channel, note, velocity
          MidiUSB.flush();
